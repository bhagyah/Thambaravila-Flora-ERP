...
export default function TargetProgressRing({
  ...
  return (
    <div className={`rounded-2xl p-5 flex h-full w-full flex-col justify-between space-y-4 border ${shell}`}>
      ...
      <div className={`flex flex-col space-y-3 border-b pb-3 ${isLight ? 'border-slate-200' : 'border-white/10'}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <div className={`flex h-8 w-8 items-center justify-center rounded-xl border text-sm shadow-inner ${isLight ? 'border-slate-200 bg-slate-50 text-slate-700' : 'border-white/10 bg-white/[0.06] text-white'}`}>
              🎯
            </div>
            <div>
          ...
          <span className={`rounded px-2 py-0.5 text-[10px] font-extrabold uppercase ${isLight ? 'border border-slate-200 bg-slate-50 text-[#4E9D82]' : 'border border-flora-sage/30 bg-flora-sage/15 text-flora-sage'}`}>
            {timeframeLabel}
          </span>
        </div>
        ...
        {onTimeRangeChange && (
          <div className={`flex items-center justify-between rounded-xl border p-1 text-[11px] font-bold ${isLight ? 'border-slate-200 bg-slate-50' : 'border-white/10 bg-white/[0.06]'}`}>
            {[
              { id: '1d', label: 'Daily' },
              { id: '1w', label: 'Weekly' },
              { id: '1m', label: 'Monthly' },
      ...
      <div className="flex-1 flex flex-col items-center justify-center space-y-4 py-2 my-auto">
        ...
        <div className="relative w-36 h-36 flex items-center justify-center flex-shrink-0">
          <svg viewBox="0 0 140 140" className="w-full h-full transform -rotate-90">
            ...
            <circle
              ...
              r={radius}
              fill="transparent"
              stroke={isLight ? '#dbe4dd' : 'var(--flora-border)'}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            <span className={`text-2xl font-black tracking-tight ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
              {clampedProgress}%
            </span>
        ...
        <div className="w-full space-y-2 text-xs">
          <div className={`flex items-center justify-between rounded-xl border p-2.5 ${isLight ? 'border-slate-200 bg-slate-50' : 'border-white/10 bg-white/[0.06]'}`}>
            <span className={`flex items-center space-x-1.5 font-semibold ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>
              <span>🎯</span>
              <span>Target Goal:</span>
            </span>
            <span className={`font-black ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>{formatLKR(targetVal, false)}</span>
          </div>

          <div className={`flex items-center justify-between rounded-xl border p-2.5 ${isLight ? 'border-emerald-200 bg-emerald-50' : 'border-emerald-500/20 bg-emerald-500/10'}`}>
            <span className={`flex items-center space-x-1.5 font-semibold ${isLight ? 'text-emerald-700' : 'text-emerald-400'}`}>
              <span>💳</span>
              <span>Achieved:</span>
            </span>
            <span className={`font-black ${isLight ? 'text-emerald-700' : 'text-emerald-300'}`}>{formatLKR(achievedVal, false)}</span>
          </div>

          <div className={`flex items-center justify-between rounded-xl border p-2.5 ${isLight ? 'border-amber-200 bg-amber-50' : 'border-amber-500/20 bg-amber-500/10'}`}>
            <span className={`flex items-center space-x-1.5 font-semibold ${isLight ? 'text-amber-700' : 'text-amber-400'}`}>
              <span>⏳</span>
              <span>Need:</span>
            </span>
            <span className={`font-black ${isLight ? 'text-amber-700' : 'text-amber-300'}`}>
