...
export default function AreaTrendChart({
  ...
  const axisColor = isLight ? '#5A625D' : '#94a3b8';
  const gridColor = isLight ? '#DDD8D3' : '#334155';
  const tooltipShell = isLight
    ? 'bg-[#FAF9F8] border-[#DDD8D3] text-[#1E2421] shadow-[0_12px_32px_rgba(40,35,30,0.1)]'
  ...
  return (
    <div className={`rounded-2xl p-6 space-y-4 flex h-full flex-col justify-between border ${shell}`}>
      ...
      <div className={`flex flex-col gap-3 border-b pb-4 md:flex-row md:items-center md:justify-between ${isLight ? 'border-[#DDD8D3]' : 'border-white/10'}`}>
        <div>
